...
const collectFormData = (inputs) => {
  const params = {}, query = {}, body = {}, formData = new FormData();
  let hasFile = false;

  for (const input of inputs) {
    const value = input.value.trim();
    if (value === '' && input.type !== 'file') { continue; }
...
const handleFormSubmit = async (event, endpoint, inputs, responseDiv) => {
  ...
  if (['/login', '/logout'].includes(endpoint.path)) {
    localStorage.removeItem('token');
    if (endpoint.path === '/logout') {
      responseDiv.textContent = 'logout complete';
      return;
    }
  }
  ...
  try {
    const res = await axios({
      ...
      url: path,
      headers,
      data: ['POST', 'PATCH', 'PUT', 'DELETE'].includes(endpoint.method)
        ? hasFile ? formData : body
        : undefined
    });

    // 로그인 처리
    if (endpoint.path === '/login' && res.data?.data?.token) {
